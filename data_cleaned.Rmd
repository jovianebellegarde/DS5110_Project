...
```{r}
set.seed(2)
merged_part <- resample_partition(merged_df,
                                    p=c(train=0.6,
                                        valid=0.2,
                                        test=0.2))
...
step1 <- function(response, predictors, candidates, partition)
{
  ...
  rmses <- sapply(formulas,
                  function(fm) rmse(glm(fm, data=partition$train),
                                    data=partition$valid), family = "binomial")
...
```{r}
preds <- "1"
cands <- c("mrace6", "cut", "color", "clarity",
          "x", "y", "z", "depth", "table")
s1 <- step1("log10(price)", preds, cands, merged_df)
