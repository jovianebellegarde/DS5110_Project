...
age_table <- data.frame(
  age_category = c("Under 15", "15-19", "20-24", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54"),
...
education_table <- data.frame(
  education_category = c("8th Grade or Less", "9-12th Grade w/out Diploma", "High School Graduate", "Some College Credits", "Associate's", "Bachelor's",
                         "Master's", "Doctorate/Professional Degree","Unknown"),
  numerical_value = c(1, 2, 3, 4, 5, 6,7,8,9)
)
...
# Balancing the overrepresentation of the Maternal White Population 
```{r}
...
```{r}
ggplot(training_data, aes(x=mrace6 )) + 
  geom_bar()+ 
...
# Distribution of Age 
  ## Notes: Race is more evenly distributed after oversampling
```{r}
ggplot(training_data, aes(x=mager9 )) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(title="Race ")
...
```{r}
ggplot(training_data, aes(x=risk_level_cat )) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(title="Risk Level ")

filtered_df <- training_data %>% filter(risk_level_cat == "3")
nrow(filtered_df)
```
