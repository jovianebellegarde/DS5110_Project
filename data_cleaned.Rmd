...
death_cases <- training[training$mortality_status == 1, ]
death_cases
alive_cases <- training[training$mortality_status == 0, ]
...
weights <- ifelse(training_data$mrace6 == 1, 1/0.758, ifelse(training_data$mrace6 == 2, 1/0.136, ifelse(training_data$mrace6 == 3, 1/0.013, ifelse(training_data$mrace6 == 4, 1/0.061, ifelse(training_data$mrace6 == 5, 1/0.003, ifelse(training_data$mrace6 == 6, 1/0.029, 1/0.758))))))
...
model_simple <- glm(mortality_status ~ mrace6, data = training_data, family = binomial)
...
model_stepwise_AIC <- stepAIC(glm(mortality_status ~ 1, data = training_data, family = binomial(link = "logit")), 
                              direction = "forward", 
                              scope = formula(~ mrace6 + risk_level + meduc + mager9), 
                              trace = FALSE, 
