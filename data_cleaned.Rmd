...
# coef_df <- data.frame(coef(summary(model_stepwise_AIC))[, 1:2])
# names(coef_df) <- c("odds_ratio", "95% CI")
# 
# # calculate the confidence intervals
# coef_df$ci_low <- exp(coef(summary(model_stepwise_AIC))[, 1] - 1.96 * coef(summary(model_stepwise_AIC))[, 2])
# coef_df$ci_high <- exp(coef(summary(model_stepwise_AIC))[, 1] + 1.96 * coef(summary(model_stepwise_AIC))[, 2])
# 
# # plot the forest plot
# forestplot(coef_df, is.summary = c(TRUE, TRUE, rep(FALSE, nrow(coef_df) - 2)), 
#            mean = log(coef_df$odds_ratio), lower = log(coef_df$ci_low), upper = log(coef_df$ci_high),
#            xticks = c(0.1, 0.25, 0.5, 1, 2.5, 5, 10), col = fpColors(box = "royalblue"), 
#            zero = 1, boxsize = 0.4, txt_gp = fpTxtGp(label = list(gpar(fontsize = 12)), 
#            ticks = list(gpar(fontsize = 10)), title = list(gpar(fontsize = 14, fontface = "bold"))))
# 
# 
# 
# ```
