...
model_stepwise_AIC <- stepAIC(glm(mortality_status ~ 1, data = imputed_partition$train, family = binomial(link = "logit")), 
                              ...
                              trace = TRUE) # add prints to all steps 
...
```{r}
library(lmtest)
# Fit the two nested logistic regression models
model1 <- glm(mortality_status ~ meduc + mager9 + risk_level_cat, 
                     family = binomial(link = "logit"), 
                     data = imputed_partition$train)
model2 <- glm(mortality_status ~ meduc + mrace6 + mager9 + risk_level_cat, 
                  family = binomial(link = "logit"), 
                  data = imputed_partition$train)
