...
```{r}
ggplot(imputed_rf, aes(x=Race_Category)) + 
  geom_bar()+ 
...
# Distribution of Age 
  ## Notes: Race is more evenly distributed after oversampling
```{r}
ggplot(imputed_rf, aes(x=Age_Category)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
ggplot(imputed_rf, aes(x=risk_level)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
ggplot(imputed_rf, aes(x=Educ_Category)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
ggplot(imputed_rf, aes(x=Age_Category, fill=Race_Category)) + 
...
model_stepwise_AIC <- stepAIC(glm(mortality_status ~ 1, data = imputed_partition$train, family = binomial(link = "logit")), 
                              direction = "forward", 
                              scope = formula(~ mrace6 + risk_level_cat + meduc + mager9, bfacil), 
                              trace = FALSE, 
