...
imputed_rf <- births %>% 
  ...
  mutate(risk_level_cat = ifelse(sumrf %in% 0:5, 1, 
                                 ifelse(sumrf %in% 6:11, 2,
...
reduced_model <- glm(mortality_status ~ meduc + mager9, 
                     family = binomial(link = "logit"), 
                     data = imputed_partition$train)

# Fitting the full model with all predictor variables
full_model <- glm(mortality_status ~ meduc + mrace6 + mager9, 
                  family = binomial(link = "logit"), 
                  data = imputed_partition$train)
...
# Conclusion: With the p-value being (1.106e-10 > 0.001), we can conclude that removing race has a significant effect on the model. 
