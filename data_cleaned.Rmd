...
library(naniar)
library(stringr)
numerator_data_test <- fread("~/Desktop/Project_Intro_Data_Management/link2020usnum.csv")

numerator_data_test
numerator_data <- fread("~/Desktop/Project_Intro_Data_Management/link2020usnum.csv", 
...
```{r}
denom_data <- fread("/Users/grazianoperegrino/Desktop/Project_Intro_Data_Management/link2020usden.csv", select=c('brthwgt','dob_tt','bfacil','pwgt_r','wtgain','mrace6','ilive','meduc','bfacil3','rf_pdiab','rf_gdiab','rf_phype','rf_ghype','rf_ehype','rf_ppb','rf_inft','rf_drg','rf_art','rf_cesar','no_risks', 'mager9', 'dob_yy', 'dob_mm','dob_wk'))
...
births <- merged_df %>% dplyr::select(-c('brthwgt.x','brthwgt.y', 'dob_tt.x','dob_tt.y',
                                                  'bfacil.x','bfacil.y', 
...
race_counts <- as.character(training_data$mrace6)
cat("Original race counts:\n", race_counts, "\n")
training_data <- training_data[]
count <- table(race_counts)
count <- data.frame(count)
count
names(count)

# Define the minority and majority races
minority_races <- table(race_counts[race_counts < max(race_counts)])
majority_race <- names(race_counts[race_counts == max(race_counts)])

minority_races <- table(race_counts)[race_counts == min(race_counts)]
majority_race <- (names(race_counts))[race_counts == max(race_counts)]
...
model_stepwise_AIC <- stepAIC(glm(mortality_status ~ 1, data = imputed_partition$train, family = binomial(link = "logit")), 
                              direction = "forward", 
                              scope = formula(~ mrace6 + risk_level_cat + meduc + mager9), 
                              trace = FALSE, 
...
for(i in 1:length(race_values)) {
  ...
  model_race <- glm(mortality_status ~ mrace6 + mager9 + meduc, data = imputed_rf, family = binomial)
