...
imputed_rf <- imputed_rf %>% 
  mutate(Age_Category = age_table$age_category[match(imputed_rf$mager9, 
                                                  age_table$numerical_value)]) %>%
    mutate(Race_Category = race_table$race_category[match(imputed_rf$mrace6, 
                                                  race_table$numerical_value)]) %>%
      mutate(Educ_Category = education_table$education_category[match(imputed_rf$meduc, 
                                                  education_table$numerical_value)])
```

1. Split the data into a training set and a testing set.
2. Use oversampling techniques (such as SMOTE) on the training set to address any class imbalance issues.
3. Use cross-validation (such as k-fold cross-validation) on the oversampled training set to build and tune the model. This involves iterating through different hyperparameters and/or subsets of variables to find the best model.
4. Use the final selected model to make predictions on the testing set.
5. Evaluate the performance of the model on the testing set using appropriate metrics (such as accuracy, precision, recall, F1 score, ROC curve, AUC, etc.).
6. Refit the model on the full original dataset (including the oversampled data), using the final selected hyperparameters and variables. This final model can then be used for making predictions on new, unseen data.

# ------------------------  DATA PARTIITIONING & OVERSAMPLING ------------------
...
# Oversampling: Balancing the Births and Deaths
```{r}
# Subset the data frame by death and alive classes
training <- imputed_partition$train$data
death_cases <- imputed_rf[training$mortality_status == 1, ]
death_cases
alive_cases <- imputed_rf[training$mortality_status == 0, ]
...
# Oversampling: Balancing the overrepresentation of the Maternal White Population
```{r}
...
# Testing the train/control -- DRAFT
```{r}
...
# testing only factor variables
test_training <- training_data %>% dplyr::select(-c(ilive, key, sumrf,risk_level,Age_Category,Race_Category,Educ_Category))

# Test 1
default_glm_mod = train(
  form = mortality_status ~ mrace6 + risk_level_cat + meduc + mager9,
  data = test_training,
  trControl = trainControl(method = "cv", number = 5),
  method = "glm",
  family = "binomial"
...
default_glm_mod2 = train(
  form = mortality_status ~ mrace6 + risk_level_cat,
  data = test_training,
  trControl = trainControl(method = "cv", number = 5),
  method = "glm",
  family = "binomial"
...
# Test 3
default_glm_mod3 = train(
  form = mortality_status ~ mrace6,
  data = test_training,
  trControl = trainControl(method = "cv", number = 5),
  method = "glm",
  family = "binomial"
...
default_glm_mod4 <- train(mortality_status ~ mrace6 , data=test_training,
...
mortality_cv <- crossv_kfold(training_data, 5)
...
# Distribution of Race
  ## Notes: Race is more evenly distributed after oversampling
```{r}
ggplot(training_data, aes(x=Race_Category)) + 
  geom_bar()+ 
...
# Distribution of Age 
  ## Notes: Race is more evenly distributed after oversampling
```{r}
ggplot(training_data, aes(x=Age_Category)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
ggplot(training_data, aes(x=risk_level)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  labs(title="Distribution of Health Risk Level")
```

# Distribution of Education
```{r}
ggplot(training_data, aes(x=Educ_Category)) + 
  geom_bar()+ 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
ggplot(training_data, aes(x=Age_Category, fill=Race_Category)) + 
...
# Age and Risk Level HeatMap
```{r}
freq_table <- table(Age = training_data$Age_Category, Risk_Level = training_data$risk_level_cat)
...
model_stepwise_AIC <- stepAIC(glm(mortality_status ~ 1, data = training_data, family = binomial(link = "logit")), 
                              direction = "forward", 
                              scope = formula(~ mrace6 + risk_level_cat + meduc + mager9), 
                              trace = FALSE, 
                              k = log(nrow(training_data)))
...
odds_ratios <- apply(sim_data, 2, function(x) {
  model_sim <- glm(x ~ mrace6 + mager9 + meduc, data = imputed_rf_oversampled, family = binomial)
