...
numerator_data <- fread("/Users/lesrene/Downloads/link2020usnum.csv", 
                      select=c('brthwgt','dob_tt', 'bfacil','pwgt_r', 'wtgain', 'dob_yy', 'dob_mm','dob_wk', 'sex')) %>% arrange(desc(brthwgt))
...
# Denominator File
denom_data <- fread("/Users/lesrene/Downloads/link2020usden.csv",select=c('brthwgt','dob_tt','bfacil','pwgt_r','wtgain','mrace6','ilive','meduc','bfacil3','rf_pdiab','rf_gdiab','rf_phype','rf_ghype','rf_ehype','rf_ppb','rf_inft','rf_drg','rf_art','rf_cesar','no_risks', 'mager9', 'dob_yy', 'dob_mm','dob_wk', 'sex'))
...
```{r}
merged_df_view <- merged_df %>%
  dplyr::select(mrace6) %>%
  count(mrace6) %>%
  rename(num_counts = n) %>%
  group_by(mrace6)
merged_df_view
ggplot(merged_df_view, aes(fill=mrace6, y=num_counts, x=mrace6)) + 
...
n_alive <- n_death 
weights <- ifelse(alive_cases$mrace6 == 1, 1/0.758, ifelse(alive_cases$mrace6 == 2, 1/0.136, ifelse(alive_cases$mrace6 == 3, 1/0.013, ifelse(alive_cases$mrace6 == 4, 1/0.061, ifelse(alive_cases$mrace6 == 5, 1/0.003, ifelse(alive_cases$mrace6 == 6, 1/0.029, 1/0.758))))))
alive_sample <- alive_cases[sample(nrow(alive_cases), n_alive, replace = FALSE, prob = weights), ]
...
```{r}
balanced_df_view <- balanced_data %>%
  dplyr::select(mrace6) %>%
  count(mrace6) %>%
  rename(num_counts = n) %>%
  group_by(mrace6)
balanced_df_view
ggplot(balanced_df_view, aes(fill=mrace6, y=num_counts, x=mrace6)) + 
    geom_bar(stat = "identity")
```
...
```{r}
# fit the logistic regression model
model <- glm(mortality_status ~ mrace6 + mager9 + meduc, data = imputed_partition$train, family = binomial)
summary(model)
```
...
```{r}
data_race <- imputed_partition$train
data_race$mrace6 <- 1
