...
# RMSE approach (will not work for logistic regression model)
```{r}
...
#                   function(fm) rmse(glm(fm, data=imputed_partition$train),
#                                     data=imputed_partition$valid), family = "binomial")
...
# Log-likelihood function approach
```{r}
step1 <- function(response, predictors, candidates, partition) {
 rhs <- paste0(paste0(predictors, collapse="+"), "+", candidates)
 formulas <- lapply(paste0(response, "~", rhs), as.formula)

 # Compute AUC-ROC for each candidate model
 auc_rocs <- sapply(formulas, function(fm) {
 model <- glm(fm, data=partition$train, family=binomial())
 pred <- predict(model, newdata=partition$valid, type="response")
 #print(response, " response")
 #print(partition$valid[[response]], " partition$valid[[response]]")
 
 roc <- pROC::roc(response=partition$valid[[1]], predictor=predictors)
 pROC::auc(roc)
 })

names(auc_rocs) <- candidates
attr(auc_rocs, " best") <- auc_rocs[which.max(auc_rocs)]
auc_rocs
}
```

```{r}
step2 <- function(response, predictors, candidates, partition) {
  # Building the logistic regression
  rhs <- paste0(paste0(predictors, collapse="+"), "+", candidates)
  formulas <- lapply(paste0(response, "~", rhs), as.formula)
  
  log_likelihoods <- sapply(formulas, function(fm) {
    model <- glm(fm, data=partition$train, family=binomial())
    logLik(model)
  })
  
  names(log_likelihoods) <- candidates
  attr(log_likelihoods, "best") <- log_likelihoods[which.max(log_likelihoods)]
  log_likelihoods
}
```
...
```{r}
preds <- "1"
# cands <- c("mrace6", "risk_level_cat", "meduc", "bfacil.x", "sex.x", "mager9")
cands <- c("mrace6", "risk_level_cat", "meduc", "mager9")
#s1 <- step1("mortality_status", preds, cands, imputed_partition)
s1 <- step2("mortality_status", preds, cands, imputed_partition)
